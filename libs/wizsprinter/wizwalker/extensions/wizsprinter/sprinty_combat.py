    ...
    async def try_execute_config(self, move_config: MoveConfig, willcasted: bool = False) -> bool | Tuple[bool, bool]:
        ...
        if is_mt:
            ...
            if isinstance(target, CombatMember):
                # Single target for a multi-target spell — default to all valid targets.
                # select(...) already resolves to a list and skips this block.
                top_effects = await cur_card.get_spell_effects()
                for e in top_effects:
                    eff_target = await e.effect_target()
                    if eff_target == EffectTarget.multi_target_enemy:
                        target = await self.get_enemies()
                        break
                    elif eff_target == EffectTarget.multi_target_friendly:
                        target = [await self.get_client_member()] + await self.get_allies()
                        break
                if not target:
                    return False
                print(f"[MT-DBG] expanded to {len(target)} targets")
